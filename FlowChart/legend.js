...
/**
 * Checks if a section is empty (has no nodes) and removes it if so.
 * Also reassigns section numbers for remaining sections.
 */
function checkAndCleanupEmptySections() {
  const parent = graph.getDefaultParent();
  const vertices = graph.getChildVertices(parent);
  
  // Get all sections that currently have nodes
  const sectionsWithNodes = new Set();
  vertices.forEach(cell => {
    const section = getSection(cell);
    sectionsWithNodes.add(section);
  });
  
  // Find sections that exist in sectionPrefs but have no nodes
  const sectionsToRemove = [];
  Object.keys(sectionPrefs).forEach(sec => {
    if (!sectionsWithNodes.has(sec)) {
      sectionsToRemove.push(parseInt(sec));
    }
  });
  
  // If there are sections to remove, reassign section numbers
  if (sectionsToRemove.length > 0) {
    // Start a graph update
    graph.getModel().beginUpdate();
    try {
      // Sort sections to remove in descending order to avoid conflicts
      sectionsToRemove.sort((a, b) => b - a);
      
      // Remove empty sections from sectionPrefs
      sectionsToRemove.forEach(secToRemove => {
        delete sectionPrefs[secToRemove.toString()];
      });
      
      // Reassign section numbers for remaining sections
      const remainingSections = Object.keys(sectionPrefs).sort((a, b) => parseInt(a) - parseInt(b));
      const newSectionPrefs = {};
      
      // Create new section numbers starting from 1
      remainingSections.forEach((oldSec, index) => {
        const newSecNum = (index + 1).toString();
        newSectionPrefs[newSecNum] = {
          ...sectionPrefs[oldSec],
          borderColor: getDefaultSectionColor(index + 1)
        };
        
        // Update all cells in this section to use the new section number
        vertices.forEach(cell => {
          if (getSection(cell) === oldSec) {
            let style = cell.style || "";
            style = style.replace(/section=[^;]+/, "");
            style += `;section=${newSecNum};`;
            graph.getModel().setStyle(cell, style);
          }
        });
      });
