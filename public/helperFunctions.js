...
// We'll rely on a global 'graph' variable that gets declared in graphInitContextMenu.js
...
// Used to highlight a node in "jump" mode
...
function getColorForQuestionType(qType) {
  switch (qType) {
    ...
    case "date":         return "#FFD700";  // goldish
    case "bigparagraph": return "#FFA07A";  // light salmon
...
/**
 * Refresh all cells to enforce correct section assignments, colors, etc.
 */
function refreshAllCells() {
  ...
  vertices.forEach(cell => {
    ...
    colorCell(cell);
    // If a newly dropped question node has default text, prompt user with a <select> (optional approach)
    if (isQuestion(cell) && 
...
// Called by the <select> element above
window.pickTypeForCell = function(cellId, val) {
  ...
  try {
    ...
    // Make label reflect the new type
    switch (val) {
      case "date":
