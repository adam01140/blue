    ...
    for (let s = 1; s < sectionCounter; s++) {
        ...
        questionsSection.forEach((questionBlock) => {
            ...
            if (questionType === 'text') {
                ...
                const checkboxOptions = []; // Collect options for conditional PDFs
                ...
                // Collect conditional PDFs for checkbox
                if (conditionalPDFEnabled) {
                    const conditionalOptionText = conditionalPDFAnswer; // This is the optionText
                    // Find the optionNameId corresponding to the optionText
                    const matchingOption = checkboxOptions.find(opt => opt.optionText === conditionalOptionText);
                    if (matchingOption) {
                        conditionalPDFs.push({
                            questionId: questionId,
                            questionNameId: matchingOption.optionNameId, // The name/id of the checkbox input
                            conditionalAnswer: matchingOption.optionValue, // The value of the checkbox input
                            pdfName: conditionalPDFName,
                            questionType: questionType
                        });
                    }
