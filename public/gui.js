 let sectionCounter = 1;
        ...
        function toggleOptions(questionId) {
            ...
            if (questionType === 'dropdown' || questionType === 'checkbox' || questionType === 'radio') {
                const optionLabel = questionType === 'radio' ? 'Yes/No Option' : 'Option';
                const optionValue = questionType === 'radio' ? ['yes', 'no'] : [];
                if (questionType === 'radio') {
                    optionValue.forEach((val) => {
                        const opt = document.createElement('option');
                        opt.value = val;
                        opt.text = `${optionLabel} ${val.charAt(0).toUpperCase() + val.slice(1)}`;
    ...
    if (jumpEnabled) {
        // Update the jump options when jump logic is enabled
...
if (jumpEnabled && jumpTo) {
    ...
    <script>
        document.getElementById('answer${questionBlock.id.replace('questionBlock', '')}').addEventListener('change', function() {
            if (this.value === '${jumpOption}') {  // This now uses the correct user-entered value
                jumpTarget = '${jumpTo}';
            } else {
